 ...
 *	evWrite(int descriptor,unsigned *data,int datalen)
 *	evRead(int descriptor,unsigned *data,int *datalen)
...
#ifndef VXWORKS
int evread_(long *handle,unsigned *buffer,int *buflen)
{
  return(evRead(*handle,buffer,*buflen));
}
#endif

int evRead(long handle,unsigned *buffer,int buflen)
{
  EVFILE *a;
...
#ifndef VXWORKS
int evwrite_(long *handle,unsigned *buffer)
{
  return(evWrite(*handle,buffer));
}
#endif

int evWrite(long handle,unsigned *buffer)
{
  EVFILE *a;
